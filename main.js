...
async function DailyPlanner() {
   ...
   let lastAlertedTime = null; 
    ...
    // Check the computer clock every 10 seconds
    setInterval(() => {
        const now = new Date();
        const currentHour = now.getHours(); 
        const currentMinute = now.getMinutes(); 
        ...
        // Scan all 18 input rows
        for(let idx = 0; idx < 18; idx++) {
            // FIX: Don't skip if the box is empty! Just default it to a blank string.
            const taskText = dayPlanData[idx] || ""; 

            // Priority 1: Check if a custom time is typed in the main task box
            let parsedTime = extractTime(taskText);
            
            // Priority 2: If no custom time, check the yellow label on the left
            if (!parsedTime) {
                const labelText = dayPlanData[`label_${idx}`] || formatAMPM(4 + idx);
                parsedTime = extractTime(labelText);
            }

            // FIRE THE NOTIFICATION if the parsed time perfectly matches current time
            if (parsedTime && currentHour === parsedTime.hour && currentMinute === parsedTime.minute) {
                if (Notification.permission === "granted") {
                    ...
                    new Notification("Daily Schedule", {
                        // If they didn't type a task name, it will just say "Scheduled Task"
...
} // <-- Make sure you keep the closing bracket for the DailyPlanner function!
    ...
    try {
        ...
        myChart = new Chart(ctx, {
            ...
            options: {
                ...
                interaction: {
                    mode: 'index',
                    intersect: false, // Makes the hover tooltip snap easier
                },
                scales: {
                    y: {
                        beginAtZero: true,
